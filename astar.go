...
// shamelessly stolen and adapted from https://github.com/beefsack/go-astar/
...
func Path(from, to Pather) (path []Coord, distance int, found bool) {
	nm := nodeMap{}
	nq := &priorityQueue{}
	heap.Init(nq)
	...
	for {
		...
		if current.pather.Coord() == to.Coord() {
			// Found a path to the goal.
			p := []Coord{}
			for curr := current; curr != nil; curr = curr.parent {
				p = append(p, curr.pather.Coord())
			}
			slices.Reverse(p)
